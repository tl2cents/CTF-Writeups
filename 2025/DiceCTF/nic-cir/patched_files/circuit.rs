// path: fancy-garbling/src/circuit.rs
...
use crate::{
    ...
    informer::Informer,
    FancyArithmetic, FancyBinary, WireMod2, CustomAND
};
...
impl<F: FancyBinary> EvaluableCircuit<F> for BinaryCircuit {
    fn eval(
        ...
        println!("Evaluating circuit done...");
        let mut outputs = Vec::with_capacity(self.noutputs());
        for r in self.get_output_refs().iter() {
            let r = cache[r.ix]
                .as_ref()
...
pub trait CustomEvaluableCircuit<F: Fancy>: CircuitType {
    /// Function to evaluate the circuit
    fn custom_eval(
        &self,
        f: &mut F,
    ...
    where F: Fancy<Item = WireMod2>;
}

impl<F: FancyBinary<Item = WireMod2> + CustomAND> CustomEvaluableCircuit<F> for BinaryCircuit {
    fn custom_eval(
        &self,
        f: &mut F,
        garbler_inputs: &[F::Item],
        ...
        for (i, gate) in self.gates.iter().enumerate() {
            ...
            let (zref_, val) = match *gate {
                ...
                BinaryGate::And {
                    ...
                    let (label, x_bit, y_bit, success) = f.leaky_and(
                        cache[xref.ix]
                            .as_ref()
                            .ok_or_else(|| F::Error::from(FancyError::UninitializedValue))?,
                        cache[yref.ix]
                    ...
                    if success {
                        println!("Gate {}: {} AND {}", i, x_bit, y_bit);
                        writeln!(file, "Gate {}: {} AND {}", i, x_bit, y_bit).unwrap();
                        writeln!(file, "Input Wire {}: {}", xref.ix, x_bit).unwrap();
                        writeln!(file, "Input Wire {}: {}", yref.ix, y_bit).unwrap(); 
                        writeln!(file, "Output Wire {}: {}", out.unwrap(), x_bit&y_bit).unwrap();
                    } 
                    // else {
                    //     writeln!(file, "Gate {}: {} AND {} failed", i, x_bit, y_bit).unwrap();
                    // }
                    (
                        out,
                        label,
