// path: fancy-garbling/src/twopac/semihonest/evaluator.rs
use crate::{
    errors::TwopacError, wire::WireLabel, AllWire, ArithmeticWire, Evaluator as Ev, Fancy,
    FancyArithmetic, FancyBinary, FancyInput, FancyReveal, WireMod2, CustomAND
};
...
impl<C: AbstractChannel, RNG, OT> CustomAND for Evaluator<C, RNG, OT, WireMod2>{
    fn leaky_and(&mut self, x: &Self::Item, y: &Self::Item, delta: &Self::Item) -> Result<(Self::Item, bool, bool, bool), Self::Error> {
        self.evaluator.leaky_and(x, y, delta).map_err(Self::Error::from)
    }
}
